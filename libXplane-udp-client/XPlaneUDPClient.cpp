...
XPlaneUDPClient::XPlaneUDPClient(std::string _server, uint16_t _port,
	...
	int iVal = 100000; // micro seconds
	unsigned int sz = sizeof(iVal);
	if (setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (char*)&iVal,	sizeof(iVal))) 
	{
	...
	if (getsockname(sock, (struct sockaddr*)&sin, &len) == -1)
		...
		printf("UDP Client on port number %d\n", ntohs(sin.sin_port));
...
void XPlaneUDPClient::listenerThread() {
	...
	while (!quitFlag) {
		...
		if ((recv_len = recvfrom(sock, buf, sizeof(buf), 0,	(sockaddr *) &remoteAddr, &slen)) < 0)
		{
		...
		if (recv_len >= 4 && buf[0] == 'R' && buf[1] == 'R' && buf[2] == 'E'
			...
			for (int idx = 5; idx < recv_len; idx += 8) {
				...
				// get value
				uint8_t* buf_ui8 = (uint8_t*)buf;
				float value = xflt2float(&buf_ui8[idx + 4]);

				// get en
				uint32_t en = xint2uint32(&buf_ui8[idx]);
...
void XPlaneUDPClient::subscribeIndividualDataRef(std::string dataRef,
	...
	if (true && needToSubscribe) 
	{
		...
		size_t res = sendto(sock, (char*) buf, sizeof(buf), 0,
				(struct sockaddr *) &serverAddr, slen);
...
void XPlaneUDPClient::sendCommand(std::string cmd) {
	char* buf = new char[cmd.length() + 6];
	memset(buf, 0, sizeof(buf));

	strcpy(buf, "CMND");
	strcpy(buf + 5, cmd.c_str());

	sendto(sock, (char *) buf, sizeof(buf), 0, (struct sockaddr *) &serverAddr,
			slen);
