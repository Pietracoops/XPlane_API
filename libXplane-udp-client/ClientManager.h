...
private:
	...
	void attachToClient(const std::string& topic, size_t publisher_index, size_t subscriber_index);
	...
	size_t storeInDeque(zmq::socket_type socket_type, std::vector<size_t>& free, std::deque<zmq::socket_t>& deque, std::deque<std::string>& s_deque);
